...
`an external project` into a repair session. The workflow did not fail because the
...
During a normal commit-and-push cycle in `an external project`:
...
To recover, `an external project` had to add thin compatibility wrappers for:
...
Adopted locally on 2026-04-16 as an upstream TheKnowledge ECR only.
An external project remains a no-VCS repository and therefore explicitly opts out
of TheKnowledge commit-helper execution and repo-root Git compatibility wrapper
installation until a future GitHub migration proposal defines that workflow.
