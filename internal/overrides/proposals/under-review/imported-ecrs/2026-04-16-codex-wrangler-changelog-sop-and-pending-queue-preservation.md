...
`project-management/proposals/approved/an external project-critical-ecr-adoption-for-local-workflow-boundaries.md`.
An external project keeps the no-VCS pending summary queue, but preserves durable
operator-visible change history in top-level `CHANGELOG.md`.
