...
real git directory under the superproject, so helpers that try to create paths
under `.git/` can crash before they finish staging, validation, or commit
work.

This is not a theoretical edge case. TheKnowledge's own Feedback workflow
expects contributors working primarily in a consuming project to file
TheKnowledge feedback from the active `TheKnowledge/` submodule. That means
...
2. A helper wants to store a cache file under the repository's real git
   directory regardless of whether `.git` is a file or a directory.
3. A future helper script needs a reusable utility for locating git-managed
   paths safely.
...
- The solution should keep relative cache locations stable from the user's
  perspective while resolving them to the correct underlying directory.
...
For paths that are conceptually rooted under `.git/`, helper code should:
1. detect that the configured path is `.git`-relative;
2. ask git for the real git-dir path;
3. rebuild the target path under that real directory; and
4. create parent directories only after resolution.

Apply that logic to `scripts/run_quality_gate_cached.py` first, because that
is the currently confirmed failure. Then review the rest of the standardized
git-helper stack for any other places that assume `.git` is a directory in the
worktree.

Where a helper cannot resolve the git directory cleanly, emit a purpose-built
...
- Risk: path handling changes subtly for standalone repositories.
  Mitigation: add tests for both ordinary checkout semantics and simulated
  submodule semantics.
- Risk: only fixing the cache helper leaves other `.git` assumptions hidden.
  Mitigation: include a focused audit of related standardized git helpers as
  part of implementation.
- Risk: diagnostics remain opaque if git-dir discovery itself fails.
...
- Add a regression test that simulates a submodule-style `.git` file and
  confirms the cached quality-gate helper no longer crashes.
- Confirm that `python scripts/git_standard_commit_push.py -m "..."` can
  complete in an active submodule checkout after the fix.
...
2. Hard-code the superproject `.git/modules/...` path shape.
   Rejected because git already exposes the authoritative git-dir path and the
   hard-coded layout would be more brittle.
3. Disable quality-gate caching in submodules.
   Rejected because the real bug is path resolution, not caching itself.
4. Document the failure without fixing it.
...
1. Review and approve or revise this ECR.
   ...
   Dependencies: review of the existing bug record and helper scope.
   Entry criteria: proposal exists under review.
   Exit criteria: operator decision recorded in the proposal.
2. Implement git-dir-aware path resolution.
   Target date: 2026-03-27.
   Owners: AI maintainers.
   Dependencies: proposal approval.
   Entry criteria: approved direction to fix standardized helper paths.
   Exit criteria: cache/helper code resolves `.git` paths correctly.
3. Validate the standardized submodule workflow.
   Target date: 2026-03-27.
   Owners: AI maintainers.
   Dependencies: implementation complete.
   Entry criteria: helper changes are merged.
...
Because the problem affects consuming-project feedback filing, call out the
...
- 2026-03-25T15:15:56-07:00 - Approved by the operator after the trunk-side
  implementation already landed in commit `16b3541`. The accepted fix
  resolves `.git/...` helper paths through `git rev-parse --git-dir`, adds
  regression coverage, and closes the originating bug.
