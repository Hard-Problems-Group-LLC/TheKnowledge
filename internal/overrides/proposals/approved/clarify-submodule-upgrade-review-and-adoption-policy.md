...
Adoption Policy
Author: Codex
Date: 2026-03-25T14:38:39-07:00
Status: Approved
...
new TheKnowledge version, maintainers should fetch the latest upstream
`trunk`, briefly review what changed relative to the project's current
submodule commit, and then treat that reviewed upstream version as the new
local and remote baseline for the consuming project.

Without that explicit policy, an agent may interpret "update TheKnowledge" too
narrowly as "pull whatever the submodule remote gives me and reconcile AGENTS
drift" without first reviewing the incoming delta. That weakens situational
...
- Require a quick review of upstream changes before a project adopts a new
  submodule version.
- Clarify that the reviewed upstream version should become the consuming
  project's new baseline locally and on its remote after validation.
...
1. An operator says "update TheKnowledge" in a consuming project and expects
   ...
   pointer.
2. A project upgrade brings in new knacks, starter-tooling changes, or
   workflow guidance that should be noticed before the maintainer resumes
   unrelated project work.
3. A consuming project wants one reviewable diff that shows the submodule
   pointer update together with any AGENTS refresh or local follow-on changes.
...
- The documentation should not assume a specific transport such as HTTPS or
  SSH, but it should still talk about the latest upstream `trunk`.
...
2. review the incoming changes relative to the consuming project's current
   submodule commit, at least at a brief summary level;
3. fast-forward or otherwise adopt that reviewed upstream version in the
   submodule checkout;
4. reconcile managed-file drift and any related local adjustments; and then
5. commit and push the consuming project's resulting submodule-pointer update
   so that the reviewed version becomes the project's new shared baseline.

The wording should make the review step visible but lightweight. A commit-log
summary, diffstat, or targeted file review is enough for routine upgrades so
long as the maintainer is not blindly moving the pointer.

Relevant documentation targets likely include:
- `README.md`
- `standards-and-practices/docs/installation.txt`
...
- Risk: projects confuse "review upstream delta" with "merge every upstream
  change immediately."
  Mitigation: state that adoption still depends on the operator's direction,
  but when an update is requested the incoming version should be reviewed
  before it becomes the new baseline.
- Risk: duplicated instructions drift across multiple docs.
...
- Perform a consuming-project smoke check: fetch newer TheKnowledge trunk,
...
2. Require full per-file review for every submodule update.
   Rejected because that is heavier than necessary for routine maintenance.
3. Leave the policy entirely to consuming-project local docs.
...
1. Review and approve or revise this ECR.
   ...
   Dependencies: review of the current README and template wording.
   Entry criteria: proposal exists under review.
   Exit criteria: operator decision recorded in the proposal.
2. Update canonical docs and managed template text.
   Target date: 2026-03-27.
   Owners: AI maintainers.
   Dependencies: proposal approval.
   Entry criteria: approved policy language.
   Exit criteria: the review-and-adopt upgrade sequence is documented.
3. Validate the consuming-project upgrade flow.
   Target date: 2026-03-27.
   Owners: AI maintainers.
   Dependencies: documentation changes complete.
...
- 2026-03-25T15:15:56-07:00 - Approved by the operator for immediate
  implementation. The adopted policy is review-before-adopt: fetch and
  briefly review the incoming upstream `trunk` delta, then adopt that
  reviewed version, reconcile managed-file drift, and commit the resulting
  submodule-pointer update as the consuming project's new shared baseline.
