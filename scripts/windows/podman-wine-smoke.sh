...
IMAGE="${PROJECT_WINE_IMAGE:-docker.io/scottyhardy/docker-wine:latest}"
PODMAN_ROOT="${PROJECT_PODMAN_ROOT:-/tmp/podman-root}"
PODMAN_RUNROOT="${PROJECT_PODMAN_RUNROOT:-/tmp/podman-runroot}"
PODMAN_TMPDIR="${PROJECT_PODMAN_TMPDIR:-/tmp/podman-tmp}"
XDG_RUNTIME="${PROJECT_PODMAN_XDG_RUNTIME:-/tmp/podman-xdg-runtime}"
OCI_RUNTIME="${PROJECT_PODMAN_OCI_RUNTIME:-/usr/bin/crun}"
...
stub_dir="$(mktemp -d /tmp/project-wine-stubs.XXXXXX)"
