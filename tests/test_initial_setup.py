...
def test_initial_setup_installs_project_management_templates(tmp_path: Path) -> None:
    ...
    assert "collision resume" in agents.read_text(encoding="utf-8")
    assert "cross-project confirmation" in agents.read_text(encoding="utf-8")
    assert ".codex-local/package.json" in agents.read_text(encoding="utf-8")
    assert "Codex and codex-wrangler paths" in agents.read_text(encoding="utf-8")
    assert "Prefer patch-style edits" in agents.read_text(encoding="utf-8")
    assert "deterministic fixtures and fakes before dynamic mocks" in (
        agents.read_text(encoding="utf-8")
    )
    assert "CHANGELOG.md" in agents.read_text(encoding="utf-8")
